  ...
  <div>
    ...
    <div class="gva-table-box">
      ...
      >
        ...
        <el-table-column align="left" label="模型供应商" prop="modelProvider" width="120">
        ...
        <el-table-column align="left" label="模型名称" prop="modelName" width="120" />
...
<script setup>
  ...
  const getTableData = async() => {
    const table = await getEgoModelList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
    if (table.code === 0) {
  ...
  const onDelete = async() => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
      ...
      const res = await deleteEgoModelByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
  ...
  // 更新行
  const updateEgoModelFunc = async(row) => {
  ...
  // 删除行
  const deleteEgoModelFunc = async (row) => {
    const res = await deleteEgoModel({ ID: row.ID })
    if (res.code === 0) {
      ElMessage({
        type: 'success',
        message: '删除成功'
